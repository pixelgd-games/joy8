...
begin
  ...
  if exists(select 1 from public.looty_game_policies p join public.games g on g.id=p.game_id where g.slug='mahjong-clash')
    or exists(select 1 from public.looty_backend_keys k join public.games g on g.id=k.game_id where g.slug='mahjong-clash')
...
select jsonb_build_object(
  ...
  'continuous',(select exists(select 1 from information_schema.columns where table_schema='public' and table_name='looty_matches' and column_name='settlement_count')),
  ...
  'temporary_owner_memberships',(select jsonb_agg(jsonb_build_object('role',r.rolname,'set',m.set_option,'inherit',m.inherit_option)) from pg_auth_members m join pg_roles r on r.oid=m.roleid where m.member=(select proowner from pg_proc where oid='public.looty_product_adapter(regprocedure,text,uuid,jsonb)'::regprocedure) and r.rolname in ('mahjong_clash_accounting_owner','mahjong_clash_lifecycle_owner')),
  'platform_can_call_adapter',has_function_privilege((select proowner from pg_proc where oid='public.looty_product_adapter(regprocedure,text,uuid,jsonb)'::regprocedure),'mahjong_clash.platform_accounting(text,uuid,jsonb)','EXECUTE')
