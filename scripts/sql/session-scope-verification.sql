select pg_get_functiondef(p.oid) as definition,
  ...
  to_regprocedure('public.looty_platform_health_v1()') is not null as health_function_present,
  jsonb_build_object(
...
from pg_proc p where p.oid='public.looty_active_session(text,text)'::regprocedure;
