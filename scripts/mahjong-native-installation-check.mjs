...
try {
  const sources = JSON.parse(await readFile(path.join(root,'server/fixtures/joy8-platform.json'),'utf8'))
  for (const source of sources) await db.exec(source.sql)
  await db.exec(`create role joy8_migrator nologin createrole;
    grant create on database postgres to joy8_migrator;
    grant usage on schema public,auth,extensions to joy8_migrator;
    grant select,references on all tables in schema public,auth to joy8_migrator;
    alter table public.games owner to joy8_migrator;
    alter table public.joy8_wallet_policies owner to joy8_migrator;
    alter function public.joy8_product_adapter(regprocedure,text,uuid,jsonb) owner to joy8_migrator;
    set role joy8_migrator`)
  ...
  await assert.rejects(db.query("select public.joy8_product_adapter('mahjong_clash.platform_accounting(text,uuid,jsonb)'::regprocedure,'invalid',null,'{}')"),/MAHJONG_ADAPTER_REQUEST/)
