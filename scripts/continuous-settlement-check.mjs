...
const native = process.env.LOOTY_TEST_ENGINE === "postgres17"
...
before(async () => {
  ...
  policy = (await one("insert into public.looty_wallet_policies(game_id,initial_credit,enabled) values($1,1000,true) returning id", [game])).id
  await db.query("insert into public.looty_game_policies(game_id,wallet_policy_id,enabled,max_entry_amount) values($1,$2,true,5000)", [game, policy])
...
async function ready() {
  ...
  const p = await one("select * from public.looty_resolve_member($1,true)", [auth])
  const s = await one("select * from public.create_game_session('test-game','POINT',3600,null,$1)", [auth])
  await db.query("select public.looty_server_session_v1($1,'exchange',$2::jsonb)", [secret, JSON.stringify({ version: 1, launch_code: s.launch_code })])
...
const state = ref => rpc("looty_match_status_v1", [secret, JSON.stringify({ version: 1, match_ref: ref }), false])
const cancel = ref => rpc("looty_match_status_v1", [secret, JSON.stringify({ version: 1, match_ref: ref }), true])
async function table() {
  const players = [await ready(), await ready()]
  const body = opening(players)
  const opened = await call("looty_open_match_v1", body)
...
describe("local continuous settlement SQL", () => {
  ...
  test("each hand posts immediately while the table remains occupied; final hand releases", async () => {
    ...
    const first = await call("looty_settle_match_v1", settlement(players, body.match_ref))
    ...
    await denied(call("looty_open_match_v1", opening([players[0]])), "LOOTY_WALLET_OCCUPIED")
    await reconcile(1)
    const last = await call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true))
    ...
    assert.equal((await one("select balance from public.looty_fee_accounts where game_id=$1", [game])).balance, "20.00")
    assert.equal((await state(body.match_ref)).settlement_count, 2)
    assert.equal((await call("looty_open_match_v1", opening([players[1]]))).state, "open")
    await reconcile(1)
  })
  ...
  test("historical retry returns its saved response without rolling back the latest state", async () => {
    ...
    assert.equal((await one("select count(*)::int n from public.looty_settlements")).n, 2)
    await denied(call("looty_settle_match_v1", { ...request, final: true }), "LOOTY_IDEMPOTENCY_CONFLICT")
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 3, true)), "LOOTY_MATCH_FINALIZED")
  })

  test("duplicates under a new key, skipped hands and out-of-order hands cannot post", async () => {
    const { players, body } = await table()
    const request = settlement(players, body.match_ref)
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 2)), "LOOTY_SETTLEMENT_SEQUENCE")
    await call("looty_settle_match_v1", request)
    await denied(call("looty_settle_match_v1", { ...request, operation_key: "duplicate-new-key" }), "LOOTY_SETTLEMENT_SEQUENCE")
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 3)), "LOOTY_SETTLEMENT_SEQUENCE")
  ...
  test("sequence and final are mandatory and strictly typed with no old-shape fallback", async () => {
    ...
    for (const value of [undefined, null, 0, -1, 1.5, "1", true, 1000000000]) {
      await denied(call("looty_settle_match_v1", { ...request, settlement_no: value }), "LOOTY_INVALID_REQUEST")
    }
    for (const value of [undefined, null, 0, "false"]) {
      await denied(call("looty_settle_match_v1", { ...request, final: value }), "LOOTY_INVALID_REQUEST")
    }
    await denied(call("looty_settle_match_v1", { ...request, extra: true }), "LOOTY_INVALID_REQUEST")
  ...
  test("expired sessions and suspended players do not interrupt existing obligations", async () => {
    const { players, body } = await table()
    await call("looty_settle_match_v1", settlement(players, body.match_ref))
    ...
    await denied(call("looty_open_match_v1", opening(players)), "LOOTY_PLAYER_INACTIVE")
    assert.equal((await call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true))).state, "settled")
  ...
  test("revoked backend key cannot continue a table; replacement game key can finish it", async () => {
    ...
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true)), "LOOTY_BACKEND_UNAUTHORIZED")
    const replacement = randomBytes(32).toString("hex")
    await db.query("insert into public.looty_backend_keys(game_id,key_hash,scopes,expires_at) values($1,public.looty_hash_secret($2),array['settle'],now()+interval '1 day')", [game, replacement])
    assert.equal((await call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true), replacement)).state, "settled")
  })

  test("a zero remaining reserve still occupies the seat and rejects further loss", async () => {
    const { players, body } = await table()
    await call("looty_settle_match_v1", { ...settlement(players, body.match_ref), entries: [entry(players[0], "-1000.00"), entry(players[1], "1000.00")] })
    assert.deepEqual(await wallet(players[0]), { balance: "0.00", locked_balance: "0.00" })
    await denied(call("looty_open_match_v1", opening([players[0]])), "LOOTY_WALLET_OCCUPIED")
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 2)), "LOOTY_INVALID_ENTRY")
    await call("looty_settle_match_v1", { ...settlement(players, body.match_ref, 2, true), entries: [] })
    assert.equal((await one("select count(*)::int n from public.looty_match_participants where released_at is null")).n, 0)
  })

  test("loss is capped by current reserve, including winnings from earlier hands", async () => {
    const { players, body } = await table()
    await call("looty_settle_match_v1", settlement(players, body.match_ref))
    await denied(call("looty_settle_match_v1", { ...settlement(players, body.match_ref, 2), entries: [entry(players[0], "-901.00"), entry(players[1], "901.00")] }), "LOOTY_INVALID_ENTRY")
    await call("looty_settle_match_v1", { ...settlement(players, body.match_ref, 2, true), entries: [entry(players[0], "1090.00"), entry(players[1], "-1090.00")] })
  ...
  test("partial reservation preserves outside available funds while rolling winnings forward", async () => {
    ...
    await call("looty_open_match_v1", body)
    await call("looty_settle_match_v1", settlement(players, body.match_ref))
  ...
  test("voiding the unfinished remainder preserves all prior hand payments", async () => {
    ...
    assert.deepEqual(await wallet(players[0]), { balance: "900.00", locked_balance: "0.00" })
    assert.equal((await one("select balance from public.looty_fee_accounts")).balance, "10.00")
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true)), "LOOTY_MATCH_FINALIZED")
    await reconcile()
  })

  test("draw can retain occupancy and a final empty settlement can close a table", async () => {
    const { players, body } = await table()
    await call("looty_settle_match_v1", { ...settlement(players, body.match_ref), entries: [] })
  ...
  test("single-hand game settles and releases using the same explicit contract", async () => {
    const { players, body } = await table()
    const first = await call("looty_settle_match_v1", settlement(players, body.match_ref, 1, true))
    assert.equal(first.state, "settled")
    assert.equal((await wallet(players[0])).locked_balance, "0.00")
    await denied(cancel(body.match_ref), "LOOTY_MATCH_FINALIZED")
  })
  ...
  test("frozen wallet rolls back the next hand but never undoes the previous hand", async () => {
    ...
    await denied(call("looty_settle_match_v1", settlement(players, body.match_ref, 2)), "LOOTY_WALLET_INACTIVE")
    assert.deepEqual(await wallet(players[0]), { balance: "900.00", locked_balance: "900.00" })
    ...
    await call("looty_settle_match_v1", settlement(players, body.match_ref, 2, true))
    await reconcile()
  })
  ...
  test("browser roles and foreign backend cannot settle; entries and rules remain validated", async () => {
    const { players, body } = await table()
    const request = settlement(players, body.match_ref)
    for (const role of ["anon", "authenticated"]) await denied(rpc("looty_settle_match_v1", [secret, JSON.stringify(request)], role), "permission denied")
    await denied(call("looty_settle_match_v1", request, "0".repeat(64)), "LOOTY_BACKEND_UNAUTHORIZED")
    await denied(call("looty_settle_match_v1", { ...request, rule_version: "wrong" }), "LOOTY_RULE_MISMATCH")
    for (const amount of [100, "100.001", "1e2"]) await denied(call("looty_settle_match_v1", { ...request, entries: [{ ...request.entries[0], amount }] }), "LOOTY_INVALID_AMOUNT")
    await denied(call("looty_settle_match_v1", { ...request, entries: [entry(players[0], "1.00")] }), "LOOTY_UNBALANCED_SETTLEMENT")
    await denied(call("looty_settle_match_v1", { ...request, entries: [{ ...request.entries[0], account_ref: randomUUID() }, ...request.entries.slice(1)] }), "LOOTY_INVALID_ENTRY")
  })
  ...
  test("AI, fee, human balances and durable product markers advance or roll back together", async () => {
    ...
    await denied(call("looty_settle_match_v1", { ...next, product_commit: { fail: true } }), "LOOTY_ADAPTER_REJECTED")
    ...
    assert.equal((await state(body.match_ref)).settlement_count, 1)
    await call("looty_settle_match_v1", next)
  ...
  test("cancellation releases updated AI holds without refunding completed transfers", async () => {
    ...
    await call("looty_open_match_v1", body)
    await call("looty_settle_match_v1", { ...settlement([human, human], body.match_ref), entries: [entry(human, "-100.00"), { kind: "product", account_ref: "bot-1", amount: "100.00", source: "gameplay" }] })
  ...
  test("all committed hand records and ledger entries stay immutable", async () => {
    const { players, body } = await table()
    await call("looty_settle_match_v1", settlement(players, body.match_ref))
    for (const sql of ["update public.looty_settlements set settlement_no=9", "delete from public.looty_settlement_entries", "update public.wallet_transactions set amount=1"]) {
  ...
  test("draft refuses to change the contract underneath an open table", async () => {
    const { players, body } = await table()
    const sql = (await memberSql("../../supabase/migrations/20260918010100_continuous_settlement.sql")).replace("begin;", "").replace(/\ncommit;\s*$/, "")
    await db.exec("savepoint migration_guard")
    await denied(db.exec(sql), "LOOTY_OPEN_MATCHES")
...
describe("PostgreSQL 17 competing connections", { skip: !native }, () => {
  ...
  test("a competing new table cannot steal a wallet between hands", async () => {
    ...
    assert.match(results[0].error.message, /LOOTY_WALLET_OCCUPIED/)
    assert.equal((await wallet(players[0])).balance, "900.00")
  })
  test("different operation keys for the same hand cannot double debit", async () => {
    ...
    assert.match(results[0].error.message, /LOOTY_SETTLEMENT_SEQUENCE/)
    assert.equal((await wallet(players[0])).balance, "900.00")
  })
  ...
  test("cancellation waits for the hand and releases only the updated reservation", async () => {
    ...
    const results = await race(settleSql, [secret, JSON.stringify(settlement(players, body.match_ref))], [["select public.looty_match_status_v1($1,$2::jsonb,true) result", [secret, JSON.stringify({ version: 1, match_ref: body.match_ref })]]])
    assert.equal(results[0].error, undefined)
  ...
  test("cancellation committed first rejects a waiting hand without posting it", async () => {
    ...
    const results = await race("select public.looty_match_status_v1($1,$2::jsonb,true)", [secret, JSON.stringify({ version: 1, match_ref: body.match_ref })], [[settleSql, [secret, JSON.stringify(settlement(players, body.match_ref, 2, true))]]])
    assert.match(results[0].error.message, /LOOTY_MATCH_FINALIZED/)
    assert.deepEqual(await wallet(players[0]), { balance: "900.00", locked_balance: "0.00" })
  })
})
