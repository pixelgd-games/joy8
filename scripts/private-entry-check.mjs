...
const launch = (id = user, origin = 'http://localhost:5173', slug = 'mahjong-clash') => one('select public.looty_create_private_session($1,$2,$3) result', [slug,id,origin])
...
before(async () => {
  ...
  member = (await one('select * from public.looty_resolve_member($1,true)',[user])).player_account_id
})
...
test('activation keeps catalog hidden, removes player allowlist and creates no funds or credentials', async () => {
  ...
  assert.equal((await one("select to_regclass('public.looty_private_players') table_name")).table_name,null)
  for (const table of ['wallet_accounts','wallet_transactions','game_sessions','looty_backend_keys']) assert.equal(Number((await one(`select count(*) n from public.${table}`)).n),0)
  assert.equal((await one("select rolcanlogin from pg_roles where rolname='mahjong_clash_runtime'")).rolcanlogin,false)
  const policy = await one('select max_entry_amount,max_participants from public.looty_game_policies where game_id=$1',[game])
...
test('wrong origin, unknown game, disabled entry and published game are rejected', () => isolated(async () => {
  for (const origin of [null,'https://looty-git.pages.dev','http://localhost:5174','http://127.0.0.1:5173']) await denied(()=>launch(user,origin),/LOOTY_PRIVATE_ENTRY_DENIED/)
  await denied(()=>launch(user,'http://localhost:5173','missing-game'),/LOOTY_PRIVATE_ENTRY_DENIED/)
  await db.query('update public.games set published=true where id=$1',[game])
  await denied(()=>launch(),/LOOTY_PRIVATE_ENTRY_DENIED/)
  await db.query('update public.games set published=false where id=$1',[game])
  await db.exec('update public.looty_private_entries set enabled=false')
  await denied(()=>launch(),/LOOTY_PRIVATE_ENTRY_DENIED/)
  assert.equal(Number((await one('select count(*) n from public.wallet_accounts')).n),0)
}))
test('any enrolled guest or registered member can enter with their own identity and wallet', () => isolated(async () => {
  ...
  for (const anonymous of [true,false]) {
    ...
    const otherMember=(await one('select * from public.looty_resolve_member($1,true)',[other])).player_account_id
...
test('suspended player or disabled wallet cannot launch', () => isolated(async () => {
  ...
  await db.exec('update public.looty_wallet_policies set enabled=false')
  await denied(()=>launch(),/LOOTY_GAME_NOT_READY/)
}))
test('browser and game roles cannot read entry configuration or issue sessions', () => isolated(async () => {
  for (const role of ['anon','authenticated','mahjong_clash_server','mahjong_clash_runtime']) {
    ...
    await denied(()=>one('select * from public.looty_private_entries'),/permission denied/)
    await denied(()=>launch(),/permission denied/)
    await db.exec('reset role')
  }
  await db.exec('set role service_role')
  await denied(()=>one("select * from public.looty_issue_game_session('mahjong-clash','POINT',3600,null,$1)",[user]),/permission denied/)
}))
test('identity-only backend exchanges once, sees zero balance and cannot open a match', () => isolated(async () => {
  const key='b'.repeat(64)
  await db.query("insert into public.looty_backend_keys(game_id,key_hash,scopes,expires_at) values($1,public.looty_hash_secret($2),array['exchange','renew'],now()+interval '1 day')",[game,key])
  const session=(await launch()).result
  const request={version:1,launch_code:session.launch_code}
  const result=(await one("select public.looty_server_session_v1($1,'exchange',$2::jsonb) result",[key,JSON.stringify(request)])).result
  ...
  await denied(()=>one("select public.looty_server_session_v1($1,'exchange',$2::jsonb)",[key,JSON.stringify(request)]),/LOOTY_SESSION_INVALID/)
  await denied(()=>one("select public.looty_backend_game($1,'open')",[key]),/LOOTY_BACKEND_UNAUTHORIZED/)
}))
test('published game keeps the public launch contract', () => isolated(async () => {
  ...
  const policy=(await one('insert into public.looty_wallet_policies(game_id,enabled) values($1,true) returning id',[publicGame])).id
  await db.query('insert into public.looty_game_policies(game_id,wallet_policy_id,enabled,max_entry_amount) values($1,$2,true,1000)',[publicGame,policy])
...
test('credential provisioning is repeatable, restricted, expiring and cannot rotate another key', async () => {
  ...
  await db.exec(bundle.sql)
  assert.deepEqual((await one('select scopes from public.looty_backend_keys')).scopes,['exchange','renew'])
  assert.equal(Number((await one('select count(*) n from public.looty_backend_keys')).n),1)
