...
before(async () => {
  ...
  await db.exec(`
    ...
    reset role;
    insert into public.joy8_product_schemas values('review_a');
    insert into public.joy8_game_policies(game_id,wallet_policy_id,enabled,max_bet_amount,max_payout_amount,product_adapter)
    select g.id,w.id,true,1000,1000,'review_a.accounting(text,uuid,jsonb)'::regprocedure
...
test("registration exists and preflight never executes the product", async () => {
...
test("invalid adapter configuration, registry removal and missing schema fail before commit", async () => {
  ...
  await denied("update public.joy8_game_policies set product_adapter='review_b.bad(text,uuid,jsonb)'::regprocedure where product_adapter is not null")
  await db.exec("insert into public.joy8_product_schemas values('review_b')")
  await denied("update public.joy8_game_policies set product_adapter='review_b.bad(text,uuid,jsonb)'::regprocedure where product_adapter is not null")
