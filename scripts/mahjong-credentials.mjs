...
export function credentialBundle({ gameId, dbHost, backendKey, password, expiresAt }) {
  ...
  const sql = `do $$
declare v_game uuid; v_hash text:=public.looty_hash_secret('${backendKey}');
begin
  select g.id into v_game from public.games g
    join public.looty_private_entries e on e.game_id=g.id
    join public.looty_game_policies p on p.game_id=g.id
  ...
  if v_game is null or '${expiresAt}'::timestamptz<=now()
    or exists(select 1 from public.looty_backend_keys where game_id=v_game and (key_hash<>v_hash or revoked_at is not null))
  ...
  if exists(select 1 from pg_roles where rolname='mahjong_clash_runtime' and rolcanlogin)
    and not exists(select 1 from public.looty_backend_keys where game_id=v_game and key_hash=v_hash) then
    raise exception 'MAHJONG_RUNTIME_ALREADY_CONFIGURED';
  end if;
  insert into public.looty_backend_keys(game_id,key_hash,scopes,expires_at)
    values(v_game,v_hash,array['exchange','renew'],'${expiresAt}') on conflict(key_hash) do nothing;
  if not exists(select 1 from public.looty_backend_keys where game_id=v_game and key_hash=v_hash
