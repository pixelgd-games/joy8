...
const numericMigration = await readFile(new URL("../supabase/migrations/20260920150000_mahjong_runtime_balance.sql", import.meta.url), "utf8")
const platformMigration = await readFile(new URL("../supabase/migrations/20260920170000_shared_point_wallet.sql", import.meta.url), "utf8")
const productMigration = await readFile(new URL("../supabase/migrations/20260920165000_mahjong_shared_point_wallet.sql", import.meta.url), "utf8")
let game, beforeHash
...
before(async () => {
  ...
  const shared = sources.findIndex(source => source.path.endsWith("20260920170000_shared_point_wallet.sql"))
  ...
  for (const source of sources.slice(cut, shared)) await db.exec(source.sql)
  ...
  const policy = (await one("insert into public.joy8_wallet_policies(game_id,enabled,initial_credit) values($1,true,0) returning id", [game])).id
  await db.query("insert into public.joy8_game_policies(game_id,wallet_policy_id,enabled,max_entry_amount,product_adapter) values($1,$2,true,1000000,'mahjong_clash.platform_accounting(text,uuid,jsonb)'::regprocedure) on conflict(game_id) do update set wallet_policy_id=excluded.wallet_policy_id,enabled=excluded.enabled,max_entry_amount=excluded.max_entry_amount,product_adapter=excluded.product_adapter", [game, policy])
  beforeHash = await fingerprint()
})
after(() => db.close())

test("the empty-accounting cutover installs one shared wallet without changing player or product data", async () => {
  await db.exec(numericMigration)
  await db.exec(productMigration)
  await db.exec(platformMigration)
  assert.equal(await fingerprint(), beforeHash)
  assert.equal(Number((await one("select count(*) n from public.joy8_wallet_policies")).n), 1)
  assert.equal(Number((await one("select count(*) n from information_schema.columns where table_schema='public' and table_name='joy8_wallet_policies' and column_name='game_id'")).n), 0)
  ...
  const exchanged = (await one("select public.joy8_server_session_v1($1,'exchange',$2) value", [key, JSON.stringify({ version: 1, launch_code: session.launch_code })])).value
  assert.equal(exchanged.wallet_scope, "platform")
  ...
  assert.deepEqual(wallet, { balance: "0.00", total_balance: "0.00", locked_balance: "0.00", match_id: null, reserved_balance: "0" })
  ...
  await db.exec(await readFile(new URL("./sql/shared-point-wallet-postflight.sql", import.meta.url), "utf8"))
