...
alter table public.joy8_game_policies
  add column reservation_mode text not null default 'capped',
  add column max_reserve_amount numeric(18,2),
  add constraint joy8_game_policies_reservation_mode_check
    check(reservation_mode in ('capped','full_balance')),
  add constraint joy8_game_policies_max_reserve_amount_check
    check(max_reserve_amount is null or max_reserve_amount>0),
  add constraint joy8_game_policies_reservation_config_check
    check((reservation_mode='capped' and max_reserve_amount is null)
      or (reservation_mode='full_balance' and funding_mode='participants' and product_adapter is not null));

alter table public.joy8_matches
  add column reservation_mode text not null default 'capped',
  add column max_reserve_amount numeric(18,2),
  add constraint joy8_matches_reservation_mode_check
    check(reservation_mode in ('capped','full_balance')),
  add constraint joy8_matches_max_reserve_amount_check
    check(max_reserve_amount is null or max_reserve_amount>0),
  add constraint joy8_matches_reservation_config_check
    check((reservation_mode='capped' and max_reserve_amount is null)
      or (reservation_mode='full_balance' and funding_mode='participants' and product_adapter is not null));
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_request->'participants') loop
    ...
    if v_amount<=0 or (v_config.reservation_mode='capped' and v_amount>v_config.max_bet_amount)
